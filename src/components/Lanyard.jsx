...
export default function Lanyard({ position = [0, 0, 30], gravity = [0, -40, 0], fov = 20, transparent = false }) {
  ...
  if (!caps.webgl || caps.reducedMotion) {
    return (
      <div
  ...
  return (
    ...
    >
      <Canvas
        ...
        gl={{ alpha: true, antialias: caps.tier === 'high', powerPreference: 'default' }}
        dpr={getEffectDpr(caps.tier)}
        onCreated={({ gl }) => {
      ...
      >
        ...
        <Physics gravity={gravity} timeStep={1 / 60} maxCcdSubsteps={1}>
          <Band />
        </Physics>
